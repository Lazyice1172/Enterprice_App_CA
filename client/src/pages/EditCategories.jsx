...
const CreateCategories = () => {
  ...
  useEffect(() => {
    setLoading(true)
    axios.get(`http://localhost:5050/categories/${categoryID}`)
      .then((res) => {
        setid(res.data.id)
        setName(res.data.name)
        setPath(res.data.path.map((item) => `${item.id}:${item.name}`).join(','))
        setSubCategories(res.data.subCategories.map((item) => `${item.id}:${item.name}`).join(','))
        setLoading(false)
      })
      .catch((err) => {
        setLoading(false)
  ...
  const handleSubmit = () => {
    ...
    setLoading(true)
    axios.put(`http://localhost:5050/categories/${categoryID}`, data)
      .then(res => {
        setLoading(false)
        navigate('/')
      })
